...
type UserService interface {
	...
	//
	CreateOrganizationRole(ctx context.Context, auth types.Principle, role string, userId uint64, orgnizationId uint64, status string) (*internal_entity.UserOrganizationRole, error)
	CreateProjectRole(ctx context.Context, auth types.Principle, userId uint64, role string, projectId uint64, status string) (*internal_entity.UserProjectRole, error)
