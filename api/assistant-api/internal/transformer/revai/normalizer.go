...
// Normalize applies Rev AI-specific text transformations.
// Rev AI does NOT support SSML, so we only normalize text without XML escaping.
func (n *revaiNormalizer) Normalize(ctx context.Context, text string) string {
	if text == "" {
		return text
	}
	...
	// NO XML escaping - Rev AI uses plain text only
	// NO SSML breaks - Rev AI doesn't support SSML
...
func (n *revaiNormalizer) removeMarkdown(input string) string {
...
func (n *revaiNormalizer) normalizeWhitespace(text string) string {
	re := regexp.MustCompile(`\s+`)
